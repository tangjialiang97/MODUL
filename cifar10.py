...
def get_cifar10(root, n_labeled, out_dataset, start_label=0,
                ...
                download=True):
    base_dataset = torchvision.datasets.CIFAR10(root, train=True, download=download)  # 载入训练数据
    train_labeled_idxs, train_unlabeled_idxs, val_idxs = train_val_split(base_dataset.targets,
                                                                         int(n_labeled / 10))  # 得到标注，未标注，验证数据的下表

    train_labeled_dataset = CIFAR10_labeled(root, train_labeled_idxs, train=True, transform=transform_train)

    train_unlabeled_dataset = CIFAR10_unlabeled(root, train_unlabeled_idxs, out_path=f'./data/{out_dataset}',
                                                train=True, transform=TransformTwice(transform_train))
    train_dataset = CIFAR10_cocnat(root, train_labeled_idxs, train_unlabeled_idxs, start_label,
                                   out_path=f'./data/{out_dataset}', train=True, transform=transform_train)

    val_dataset = CIFAR10_labeled(root, val_idxs, train=True, transform=transform_val, download=True)
    test_dataset = CIFAR10_labeled(root, train=False, transform=transform_val, download=True)

    print(
        f"#Labeled: {len(train_labeled_idxs)} #Unlabeled: {len(train_unlabeled_idxs)} #OOD: {len(train_unlabeled_dataset.outpath)} #Val: {len(val_idxs)}")
